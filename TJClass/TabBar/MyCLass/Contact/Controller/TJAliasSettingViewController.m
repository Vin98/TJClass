//
//  TJAliasSettingViewController.m
//  TJClass
//
...
@interface TJAliasSettingViewController()
...
@implementation TJAliasSettingViewController
...
- (void)onDone:(id)sender{
    ...
    if (self.alias.length > self.inputLimit) {
        [SVProgressHUD showInfoWithStatus:@"备注名过长"];
    ...
    [[NIMSDK sharedSDK].userManager updateUser:self.user completion:^(NSError *error) {
        strongify(self);
        if (!error) {
            [SVProgressHUD showSuccessWithStatus:@"备注名设置成功"];
            [SVProgressHUD dismissWithDelay:1.f completion:^{
                [self.navigationController popViewControllerAnimated:YES];
            }];
            
        }else{
            [SVProgressHUD showErrorWithStatus:@"备注名设置失败，请重试"];
            [SVProgressHUD dismissWithDelay:2.f];
        }
    }];
}
